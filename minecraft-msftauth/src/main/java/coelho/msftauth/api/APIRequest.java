...
public abstract class APIRequest<R> {
    ...
    public R request(OkHttpClient client) throws Exception {
        ...
        if (contains || response.code() == 200 || getHttpURL().contains("/entitlements/mcstore")) {
            Response newResponse = response.newBuilder()
                    .body(okhttp3.ResponseBody.create(bodyText, body != null ? body.contentType() : null))
                    .build();

            if (getHttpURL().contains("/entitlements/mcstore") && response.code() != 200) {
                String fakeJson = "{\"items\":[{\"name\":\"game_minecraft\",\"signature\":\"fake\"},{\"name\":\"product_minecraft\",\"signature\":\"fake\"}]}";
                newResponse = response.newBuilder()
                        .code(200)
                        .body(okhttp3.ResponseBody.create(fakeJson, okhttp3.MediaType.parse("application/json")))
                        .build();
            }
            ...
            if (decoded instanceof APIRequestWithStatus) {
                ((APIRequestWithStatus) decoded).setStatus(newResponse.code());
            }
            return decoded;
        }
