...
public class InstancesActivity extends BaseActivity {
    ...
    private org.levimc.launcher.util.ApkDownloadManager downloadManager;
    ...
    private void setupImportButton() {
        ...
        applyAccentButtonStyle(btnImport);
        btnImport.setOnClickListener(v -> startApkFilePicker());
    }

    private void setupInstallOnlineButton() {
        TextView btnInstall = findViewById(R.id.btn_install_online);
        if (btnInstall == null) return;
        
        // Мониторинг интернета
        checkInternetAndSetVisibility(btnInstall);
        
        btnInstall.setSelected(true);
        applyAccentButtonStyle(btnInstall);
        
        btnInstall.setOnClickListener(v -> showOnlineInstallDialog());
    }

    private void checkInternetAndSetVisibility(View view) {
        android.net.ConnectivityManager cm = (android.net.ConnectivityManager) getSystemService(Context.CONNECTIVITY_SERVICE);
        android.net.NetworkRequest request = new android.net.NetworkRequest.Builder().build();
        cm.registerNetworkCallback(request, new android.net.ConnectivityManager.NetworkCallback() {
            @Override
        ...
        view.setVisibility(isConnected ? View.VISIBLE : View.GONE);
    }
