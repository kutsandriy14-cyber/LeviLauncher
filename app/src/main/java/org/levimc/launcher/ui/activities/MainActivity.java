        ...
        binding.getRoot().postDelayed(() -> {
            ...
            try {
                if (!BuildConfig.UPDATE_GITHUB_OWNER.isEmpty() && !BuildConfig.UPDATE_GITHUB_REPO.isEmpty()) {
                    new GithubReleaseUpdater(this, BuildConfig.UPDATE_GITHUB_OWNER, BuildConfig.UPDATE_GITHUB_REPO, permissionResultLauncher).checkUpdateOnLaunch();
                }
            } catch (Throwable error) {
                android.util.Log.w("MainActivity", "Update check skipped", error);
            }
        }, 1500L);
        showEulaIfNeeded();
